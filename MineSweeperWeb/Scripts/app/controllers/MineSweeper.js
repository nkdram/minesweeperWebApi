...
myApp.controller('MineSweeperController', function ($scope, $http, $interval, $uibModal) {

    $scope.reload = function () {
        $scope.check = "Test";
        $scope.gameStatus = ['start', 'ingame', 'crash', 'mine'];

        $scope.currentStatus = $scope.gameStatus[0];
        $scope.mineCount = 0;
        $scope.boxColor = randomColor();
    };
    ...
    $scope.getMines = function (strdifficulty) {

        var Jsondata = JSON.stringify({ difficulty: strdifficulty });

        $http.post('/Templates/Default.aspx/getMines', Jsondata)
       .success(function (data, status, headers, config) {
           $scope.mines = JSON.parse(data.d);

           //Set Default values after load
           $scope.mineCount = $scope.mines.mineCount;
           $scope.maxX = $scope.mines.maxX;
           $scope.maxY = $scope.mines.maxY;
           $scope.currentStatus = $scope.gameStatus[1];
   ...
   .error(function (data, status, headers, config) {
    ...
    $scope.beginGame = function (difficulty) {
        $scope.getMines(difficulty);
    };
